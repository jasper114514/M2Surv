...
class SurvivalDataset(Dataset):
    ...
    def _load_wsi_embs_from_path(self, data_dir, slide_ids):
        ...
        for slide_id in slide_ids:
            try:
                graph_path = os.path.join(data_dir, 'graph_files', '{}.pt'.format(slide_id.rstrip('.svs')))
                # print(graph_path)
                graph = torch.load(graph_path)
                ff_patch_features.append(graph.ff)
                ffpe_patch_features.append(graph.ffpe)
                graph_representations.append(graph)
                
            except FileNotFoundError:
                print('FileNotFound: ', graph_path)
                continue
        #try:
        
            
        if ffpe_patch_features.size(0) > self.num_patches:
        # 采样FFPE节点
            indices = np.random.choice(ffpe_patch_features.size(0), self.num_patches, replace=False)
            indices_set_ffpe = list(indices)
            ffpe_patch_features = ffpe_patch_features[indices]
    
            # 处理FFPE的两种边
            adj_matrix_index = graph.ffpe_edge_index.cpu().numpy()
            adj_matrix_latent = graph.ffpe_edge_latent.cpu().numpy()
    
            mask_index = np.isin(adj_matrix_index[0], indices_set_ffpe) & np.isin(adj_matrix_index[1], indices_set_ffpe)
            mask_latent = np.isin(adj_matrix_latent[0], indices_set_ffpe) & np.isin(adj_matrix_latent[1], indices_set_ffpe)
    
            adj_matrix_index = adj_matrix_index[:, mask_index]
            adj_matrix_latent = adj_matrix_latent[:, mask_latent]
    
            # 创建FFPE节点映射
            ffpe_node_map = {old: new for new, old in enumerate(indices_set_ffpe)}
            if adj_matrix_index.shape[1]:
                adj_matrix_index[0] = np.vectorize(ffpe_node_map.get)(adj_matrix_index[0])
                adj_matrix_index[1] = np.vectorize(ffpe_node_map.get)(adj_matrix_index[1])
            if adj_matrix_latent.shape[1]:
                adj_matrix_latent[0] = np.vectorize(ffpe_node_map.get)(adj_matrix_latent[0])
                adj_matrix_latent[1] = np.vectorize(ffpe_node_map.get)(adj_matrix_latent[1])
    
            graph_representations[0].ffpe_edge_index = torch.tensor(adj_matrix_index, dtype=torch.long)
            graph_representations[0].ffpe_edge_latent = torch.tensor(adj_matrix_latent, dtype=torch.long)

        # 处理FF节点和边
        if ff_patch_features.size(0) > self.num_patches:
            # 采样FF节点
            indices = np.random.choice(ff_patch_features.size(0), self.num_patches, replace=False)
            indices_set_ff = list(indices)
            ff_patch_features = ff_patch_features[indices]
    
            # 处理FF的两种边
            adj_matrix_index = graph.ff_edge_index.cpu().numpy()
            adj_matrix_latent = graph.ff_edge_latent.cpu().numpy()
    
            mask_index = np.isin(adj_matrix_index[0], indices_set_ff) & np.isin(adj_matrix_index[1], indices_set_ff)
            mask_latent = np.isin(adj_matrix_latent[0], indices_set_ff) & np.isin(adj_matrix_latent[1], indices_set_ff)
    
            adj_matrix_index = adj_matrix_index[:, mask_index]
            adj_matrix_latent = adj_matrix_latent[:, mask_latent]
    
            # 创建FF节点映射
            ff_node_map = {old: new for new, old in enumerate(indices_set_ff)}
            if adj_matrix_index.shape[1]:
                adj_matrix_index[0] = np.vectorize(ff_node_map.get)(adj_matrix_index[0])
                adj_matrix_index[1] = np.vectorize(ff_node_map.get)(adj_matrix_index[1])
            if adj_matrix_latent.shape[1]:
                adj_matrix_latent[0] = np.vectorize(ff_node_map.get)(adj_matrix_latent[0])
                adj_matrix_latent[1] = np.vectorize(ff_node_map.get)(adj_matrix_latent[1])
    
            graph_representations[0].ff_edge_index = torch.tensor(adj_matrix_index, dtype=torch.long)
            graph_representations[0].ff_edge_latent = torch.tensor(adj_matrix_latent, dtype=torch.long)

        # 处理共享边（同时需要FF和FFPE的映射）
        if hasattr(graph, 'share_edge') and graph.share_edge is not None:
            share_edge = graph.share_edge.cpu().numpy()
    
            # 确保我们有映射字典
            if ff_node_map is None:
                ff_node_map = {i: i for i in range(ff_patch_features.size(0))}
                indices_set_ff = list(range(ff_patch_features.size(0)))
            if ffpe_node_map is None:
                ffpe_node_map = {i: i for i in range(ffpe_patch_features.size(0))}
                indices_set_ffpe = list(range(ffpe_patch_features.size(0)))
    
            # 创建共享边掩码（FF节点在采样集中且FFPE节点在采样集中）
            mask_share = (
                np.isin(share_edge[0], indices_set_ff) & 
                np.isin(share_edge[1], indices_set_ffpe)
            )
            share_edge = share_edge[:, mask_share]
    
            # 重新映射节点编号
            if share_edge.shape[1] > 0:
                share_edge[0] = np.vectorize(ff_node_map.get)(share_edge[0])  # 映射FF节点
                share_edge[1] = np.vectorize(ffpe_node_map.get)(share_edge[1])  # 映射FFPE节点
    
            graph_representations[0].share_edge = torch.tensor(share_edge, dtype=torch.long)
        return ff_patch_features, ffpe_patch_features, graph_representations
    ...
    def _load_wsi_embs_from_sampled(self, data_dir, slide_ids):
        ...
        try:
            ...
            # Store FF and FFPE features
            ff_feat = graph.ff
            ffpe_feat = graph.ffpe
            graph_representations.append(graph)
        except FileNotFoundError:
            print(f'FileNotFound: {graph_path}')
            return None, None, []
        # print(graph_representations[0])
        return ff_feat, ffpe_feat, graph_representations
